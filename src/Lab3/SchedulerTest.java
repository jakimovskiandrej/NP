...
class Timestamp<T> implements Comparable<Timestamp<T>> {
    ...
    @Override
    public int hashCode() {
        int result = Objects.hashCode(time);
...
class Scheduler<T> {
    ...
    public Timestamp<T> next() {
        ...
        return elements.stream().sorted(Timestamp::compareTo).filter(t->t.getTime().isAfter(time)).findFirst().orElse(null);
    }
    //last():Timestamp<T> - го враќа објектот кој има време најблиску до тековното (сега) и веќе E поминат
    public Timestamp<T> last() {
        LocalDateTime time = LocalDateTime.now();
        return elements.stream().sorted(Timestamp::compareTo).filter(t->t.getTime().isBefore(time)).findFirst().orElse(null);
    }
    ...
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end) {
        return elements.stream().sorted(Timestamp::compareTo).filter(t->t.getTime().isBefore(begin) && t.getTime().isAfter(end)).collect(Collectors.toList());
    }
