...
class Scheduler<T> {
    ...
    public boolean remove(Date d) {
        boolean flag = schedule.containsKey(d);
        schedule.remove(d);
        return flag;
    }

    public T next() {
        Date curr = schedule.keySet().stream()
                .filter(d->d.compareTo(new Date()) > 0)
                .findFirst().get();
        return schedule.get(curr);
    }
    ...
    public ArrayList<T> getAll(Date begin, Date end) {
        return schedule.entrySet().stream()
                .filter(es->es.getKey().equals(begin) || es.getKey().equals(end)
                        || (es.getKey().after(begin) && es.getKey().before(end)))
                .map(i -> i.getValue())
                .collect(Collectors.toCollection(ArrayList::new));
    }

    public T getFirst() {
        return schedule.entrySet().stream().findFirst().get().getValue();
    }

    public T getLast() {
        return schedule.entrySet().stream().reduce((i,j) -> j).get().getValue();
    }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(schedule, scheduler.schedule);
    }
